  ...
  EventProcessors = {
    'conversation.item.created': (event) => {
      ...
      if (!newItem.start_time) {
        newItem.start_time = new Date().toISOString();
      }
    ...
    'response.created': (event) => {
      ...
      if (!this.responseLookup[response.id]) {
        ...
        if (!this.responseLookup[response.id].start_time) {
          this.responseLookup[response.id].start_time = new Date().toISOString();
        }
    ...
    'response.output_item.done': (event) => {
      ...
      if (!foundItem.end_time) {
        foundItem.end_time = new Date().toISOString();
      }
